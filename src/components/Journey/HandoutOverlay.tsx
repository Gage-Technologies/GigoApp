...
const HandoutOverlay: React.FC<HandoutOverlayProps> = ({
  ...
  const handlePanGesture = (gestureState: PanResponderGestureState) => {
    ...
    const newPosition = Math.max(
      0,
      Math.min(slideAnim._value + gestureState.dy * SCROLL_SMOOTHING, height),
    );
    if (newPosition <= 0) {
  ...
  const panResponder = PanResponder.create({
    ...
    onPanResponderRelease: async (_, gestureState) => {
      // @ts-ignore
      let snapClose =
        gestureState.dy * SCROLL_SMOOTHING + slideAnim._value > 10;
      ...
      if (
        // @ts-ignore
        gestureState.dy * SCROLL_SMOOTHING + slideAnim._value <
          height * LOWER_BOUND_SNAP_THRESHOLD ||
        // @ts-ignore
        ...
        gestureState.dy * SCROLL_SMOOTHING + slideAnim._value <
        height * UPPER_BOUND_SNAP_THRESHOLD
      ) {
        // snap to top
        Animated.timing(slideAnim, {
          toValue: 0,
